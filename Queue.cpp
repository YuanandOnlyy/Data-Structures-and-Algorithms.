...
// Simulate auto-dequeue every minute
void autoDequeueSimulation(Queue &queue) {
    while (true) {
        this_thread::sleep_for(chrono::seconds(60)); // Wait for 1 minute
        queue.autoDequeue();
    }
}
...
int main() {
    ...
    // Detach the auto-dequeue thread to allow it to run independently
